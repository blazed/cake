          ...
          value = {
            ...
            serviceConfig = {
              ...
              ExecStartPre = pkgs.writeShellScript "innernet-systemd-server-pre-${server.settings.networkName}" ''
                ...
                ${concatStringsSep "\n" (
                  map addCidr (
                    mapAttrsToList (_: v: v // { inherit (server.settings) networkName; }) server.settings.cidrs
                  )
                )}
                ${concatStringsSep "\n" (
                  map addPeer (
                    mapAttrsToList (_: v: v // { inherit (server.settings) networkName; }) server.settings.peers
                  )
                )}
              '';
