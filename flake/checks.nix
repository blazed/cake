{ inputs, ... }:
{
  perSystem =
    { pkgs, system, ... }:
    let
      piNode = import ../users/profiles/pi/node-package.nix { inherit pkgs inputs; };
    in
    {
      checks = pkgs.lib.optionalAttrs (system == "x86_64-linux") {
        ...
        pi-extensions = pkgs.callPackage ../users/profiles/pi/extension-check.nix { inherit piNode; };
      };
    };
}
