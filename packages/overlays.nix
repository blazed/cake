{
  ...
  wayland120 = pkgs:
    pkgs.wayland.overrideAttrs (old: {
      version = "1.20.0";
...
in
  ...
  // {
    ...
    wlroots = final: prev: {
      wlroots = prev.callpackage ./wlroots {
  ...
  // {
    wlroots = final: prev: {
      wlroots = prev.callPackage ./wlroots {
        libdrm = final.libdrm24109;
        wayland = final.wayland120;
        wayland-protocols = final.wayland-protocols-master;
        meson = prev.meson-060;
      };
    };
    sway-unwrapped = final: prev: {
      sway-unwrapped = prev.callPackage ./sway {
    ...
    swaylock = final: prev: {
      swaylock = prev.callPackage ./swaylock {
        wayland = final.wayland120;
        wayland-protocols = final.wayland-protocols-master;
        meson = prev.meson-060;
      };
