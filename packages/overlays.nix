...
{
  wlroots = (final: prev: { wlroots = prev.callPackage ./wlroots { libdrm = final.libdrm24109; wayland = final.wayland120; wayland-protocols = final.wayland-protocols-master; meson = prev.meson-060; }; });
  sway-unwrapped = (final: prev: { sway-unwrapped = prev.callPackage ./sway { wayland = final.wayland120; wayland-protocols = final.wayland-protocols-master; meson = prev.meson-060; }; });
  swaylock = (final: prev: { swaylock = prev.callPackage ./swaylock { wayland = final.wayland120; wayland-protocols = final.wayland-protocols-master; meson = prev.meson-060; }; });
}
