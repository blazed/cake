...
let
  ...
  mbr = if disk.dosLabel then "true" else "false";
  ...
  text = ''
    ...
    ${
      ...
      ''
        ...
        bcachefs format --discard --compression=zstd --replicas=${toString (builtins.length bcacheFsDisks)} -L root ${
          concatStringsSep " " (lib.imap1 (idx: disk: ''--label=ssd.ssd${toString idx} ${disk}'') disks)
        }
