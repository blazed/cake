...
function M.setup()
  ...
  plugin.setup {
    ...
    pickers = {
      buffers = {
        mappings = {
          n = { ["<M-BS>"] = actions.delete_buffer },
          i = { ["<M-BS>"] = actions.delete_buffer },
        },
      },
    },
...
function M.keymaps()
  K.map { "<C-e>", "Open file browser", m.open_file_browser, mode = { "n" } }

  K.mapseq { "<leader>t", "Open file finder", m.open_file_finder, mode = { "n" } }
  K.mapseq { "<leader>s", "Open project-wide text search", m.open_text_finder, mode = { "n" } }

  K.mapseq { "<leader>tc", "Open command finder", m.open_command_finder, mode = { "n" } }
  K.mapseq { "<leader>th", "Open highlight finder", "<Cmd>Telescope highlights<CR>", mode = { "n" } }
